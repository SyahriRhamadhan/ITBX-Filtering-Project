...
// Membaca data BSB zones
const bsbZones = JSON.parse(fs.readFileSync('bsb-zones-clean.json', 'utf8'));
console.log('Zone data:', Object.keys(bsbZones.zoneGroups).length, 'zone groups');

// Membaca data BSB
const bsbData = JSON.parse(fs.readFileSync('../bsb-data.json', 'utf8'));
console.log('BSB data:', bsbData.activities?.length || 0, 'activities');

// Membuat mapping kodunk ke nama zona
const kodunkToZone = {};
for (const [groupKey, zones] of Object.entries(bsbZones.zoneGroups)) {
  if (Array.isArray(zones)) {
    zones.forEach(zone => {
      kodunkToZone[zone.code] = zone.name;
    });
  }
}
...
bsbData.activities?.forEach(activity => {
  ...
  Object.entries(activity.zones).forEach(([zoneName, permission]) => {
    ...
    // Debug logging untuk permission I pada zona tertentu
    if (permission === 'I' && (zoneName === "Cagar Budaya" || zoneName === "Perdagangan dan Jasa Skala Kota")) {
      console.log(`DEBUG: Found permission I for ${zoneName} - Activity: ${activityName}`);
    }
    
    // Cari semua kodunk berdasarkan nama zona dengan berbagai variasi
    let matchingKodunks = [];
    
    // Coba cari exact match dulu
    matchingKodunks = Object.keys(kodunkToZone).filter(key => 
      kodunkToZone[key] === zoneName
    );
    
    // Jika tidak ditemukan, coba tanpa kata "Zona"
    if (matchingKodunks.length === 0) {
      const zoneNameWithoutZona = zoneName.replace(/^Zona\s+/, '');
      matchingKodunks = Object.keys(kodunkToZone).filter(key => 
    ...
    if (matchingKodunks.length === 0) {
      const zoneNameWithZona = `Zona ${zoneName}`;
      matchingKodunks = Object.keys(kodunkToZone).filter(key => 
    ...
    if (matchingKodunks.length === 0) {
      matchingKodunks = Object.keys(kodunkToZone).filter(key => {
        ...
        const searchName = zoneName.toLowerCase();
    ...
    matchingKodunks.forEach(kodunk => {
        ...
        if (zoneObj) {
          ...
          // Debug logging untuk zona tertentu
          if (zoneName === "Cagar Budaya" || zoneName === "Perdagangan dan Jasa Skala Kota") {
            console.log(`DEBUG: Processing ${zoneName} - Activity: ${activityName} - Permission: ${permission} - Kodunk: ${kodunk}`);
          }
    ...
    if (matchingKodunks.length === 0) {
        ...
        unmappedZones.add(zoneName);
        console.log(`Warning: Zona tidak ditemukan mapping untuk: "${zoneName}"`);
    }
  });
});
...
fs.writeFileSync('bsb-converted.json', JSON.stringify(convertedData, null, 2));
console.log('\nData berhasil disimpan ke bsb-converted.json');