...
function processKepsusData(workbook, sheetName) {
  console.log('\n=== PROCESSING KEPSUS DATA ===');
  ...
  for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
    ...
    const activity = {
      ...
      metadata: {
        tabel: tabel ? tabel.toString().trim() : 'TABEL KETENTUAN KHUSUS',
  ...
  console.log(`Generated ${Object.keys(regulations).length} regulations`);
...
function main() {
  try {
    console.log('Starting Kepsus Excel analysis...');
    
    // Read the Excel file
    const excelPath = path.join(__dirname, 'app', 'data', 'kepsus', 'kepsus.xlsx');
    ...
    // Process the data
    const result = processKepsusData(workbook, sheetName);
    
    // Save to JSON file
    const outputPath = path.join(__dirname, 'app', 'data', 'kepsus-data.json');
    ...
    console.log(`Activities processed: ${result.activities.length}`);
    console.log(`Regulations generated: ${Object.keys(result.regulations).length}`);
